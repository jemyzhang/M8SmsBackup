...
MZ_IMPLEMENT_DYNAMIC(UiFileDialog)

BOOL UiFileDialog::OnInitDialog(){
...
void UiFileDialog::OnMzCommand(WPARAM wParam, LPARAM lParam) {
...
void UiFileDialog::SetTitle(LPCTSTR text){
	if(text){
		m_DialogTitle.SetText(text);
	}
}
void UiFileDialog::SetInitFileName(LPCTSTR text){
	if(text){
		C::newstrcpy(&_filename,text);
		m_FileName.SetText(text);
	}
}
void UiFileDialog::SetInitFileSuffix(LPCTSTR text){
	if(text){
		C::newstrcpy(&_filesuffix,text);
		m_FileName.SetTip2(text,true);
	}
}
void UiFileDialog::SetInitFolder(LPCTSTR text){
	if(text){
...
文件名中不允许使用下列字符：< > / \ | : " * ?
*/
bool UiFileDialog::checkFileName(){
	bool bRet = false;
...
*/
bool UiFileDialog::checkFileExist(){
	return File::FileExists(GetFullFileName());
}

LPTSTR UiFileDialog::GetFullFileName() { 
